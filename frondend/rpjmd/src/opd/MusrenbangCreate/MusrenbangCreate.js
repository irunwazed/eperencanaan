...
class MusrenbangCreate extends Component {
  ...
  handleChange = idx => evt => {
    ...
    if(no === 3){
      ...
      this.getRkpd(this.state.dataPilih.Kd_Urusan, this.state.dataPilih.Kd_Bidang, this.state.dataPilih.Kd_Prog, kode[1]);
      // console.log(kode);
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        if (idx !== sidx) return shareholder;
        let data = { ...shareholder, Kd_Prog: kode[0], Kd_Keg: kode[1] };
        this.setState({dataPilih:data});
        return data;
      });
      this.setState({ dataAll: newdataTests });
  ...
  if(set){
    return (
      <div>
        ...
        <Input type="select" style={this.styleForm} value={data.Kd_Prog+"-"+data.Kd_Keg} data-number="3" onChange={this.handleChange(key)}>
          <option key="-1" value="">-= Pilih Kegiatan =-</option>
          {this.state.dataKegiatan.map((data, key) => {
            return data ? (
              <option key={key} value={data.Kd_Prog+"-"+data.Kd_Keg}>{data.Ket_Kegiatan}</option>
            ) : (null);
          })}
        </Input>
