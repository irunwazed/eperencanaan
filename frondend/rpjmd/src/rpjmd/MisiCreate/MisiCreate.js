...
class MisiCreate extends Component {
  ...
  handleChange(event){
    ...
    if(parseInt(no)===1){
      setData.misi_nama = event.target.value;
    }
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'rpjmd/menyusun/misi/page-'+page, data)
    .then(response => {
...
// submit
  handleSubmit = event => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'rpjmd/menyusun/misi/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'rpjmd/menyusun/misi/create';
    }
    ...
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('rpjmd', localStorage.getItem('codexv-rpjmd'));
    data.append('id', this.state.dataPilih.misi_id);
    data.append('misi_nama', this.state.dataPilih.misi_nama);
    axios
    .post(link, data, head)
    .then(response => {
  ...
  handleDelete = () =>{
    ...
    data.append('id', this.state.dataPilih.misi_id);
    axios
    .post(config.apiRoot+'rpjmd/menyusun/misi/delete', data)
    .then(response => {
      if(response.data.status){
        this.modalDelete();
        this.getData();
    ...
    if(this.state.loading){
      ...
      return(
        <div>
          ...
          <Row>
            <Col xs="128" md="10">
            <Download link="rpjmd/menyusun/misi"/>
          ...
          <thead style={{textAlign: 'center', backgroundColor:'#0066ff', color: 'white'}}>
            ...
            {this.state.dataAll.map((data, key) => {
              return data ? (
                <tr key={((this.state.page-1)*20)+key}>
                  <td>{key+1}</td>
                  ...
                  <Form method="post"  onSubmit={this.handleSubmit} className="form-horizontal" id="form-data">
                    <ModalBody>
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Input type="text" id="visi" data-number="1" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.misi_nama} required autoFocus/>
                          <FormText color="muted">Isi Misi</FormText>
                        </Col>
                      </FormGroup>
                    </ModalBody>
                    <ModalFooter>
