...
class VisiPenjelasan extends Component {
  
  constructor(props) {
    super(props);

    this.dataPilihAwal = {
      visi_penjelasan_id:0,
      visi_penjelasan_nama:'',
    };
    ...
    // document.title = "Menyusun Visi Penjelasan";
  ...
  handleChange(event){
    ...
    if(parseInt(no)===1){
      setData.visi_penjelasan_nama = event.target.value;
    }
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'rpjmd/menyusun/visi-penjelasan/page-'+page, data)
    .then(response => {
...
// submit
  handleSubmit = event => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'rpjmd/menyusun/visi-penjelasan/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'rpjmd/menyusun/visi-penjelasan/create';
    }
    ...
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('rpjmd', localStorage.getItem('codexv-rpjmd'));
    data.append('id', this.state.dataPilih.visi_penjelasan_id);
    
    data.append('visi_penjelasan_nama', this.state.dataPilih.visi_penjelasan_nama);
    axios
    .post(link, data, head)
    .then(response => {
  ...
  handleDelete = () =>{
    ...
    data.append('id', this.state.dataPilih.visi_penjelasan_id);
    axios
    .post(config.apiRoot+'rpjmd/menyusun/visi-penjelasan/delete', data)
    .then(response => {
      if(response.data.status){
        this.modalDelete();
        this.getData();
        ...
        <div className="animated fadeIn">
          <Row>
            <Col xs="12" lg="12">
              <Card>
                <CardHeader>
                  <i className="fa fa-align-justify"></i> Visi Penjelasan
                </CardHeader>
                <CardBody>
                  <h3 style={{textAlign: 'center',}}>Visi Penjelasan</h3>
                  <hr/>
                  {this.state.pesan}
                  <Modal isOpen={this.state.modalCreate} toggle={this.modalCreateClose} className={'modal-lg ' + this.props.className}>
                    ...
                    <Form method="post"  onSubmit={this.handleSubmit} className="form-horizontal" id="form-data">
                      <ModalBody>
                        <FormGroup row>
                          <Col md="3">
                            <Label htmlFor="text-input">Visi Penjelasan *</Label>
                          </Col>
                          <Col xs="12" md="9">
                            <Input type="text" data-number="1" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.visi_penjelasan_nama} required autoFocus/>
                            <FormText color="muted">Isi Penjelasan</FormText>
                          </Col>
                        </FormGroup>
                      </ModalBody>
                      <ModalFooter>
                  ...
                  <Row>
                  <Col xs="128" md="10">
                  <Download link="rpjmd/menyusun/visi-penjelasan"/>
                  ...
                  <Table responsive striped bordered>
                    ...
                    {this.state.dataAll.map((data, key) => {
                      return data ? (
                        <tr key={key}>
                          <td>{data.visi_penjelasan_nama}</td>
                          <td>
