...
class SubUnitCreate extends Component {
  ...
  handleChange(event){
    ...
    if(no===1){
      setData.Nm_Sub_Unit = event.target.value;
    }else if(no===2){
      ...
      setData.Kd_Unit = event.target.value;
    }else if(no===7){
...
/// set data
  ...
  getUnit = () => {
    ...
    let data = new FormData();
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('urusan', this.state.dataPilih.Kd_Urusan);
    data.append('bidang', this.state.dataPilih.Kd_Bidang);
    axios
    .post(config.apiRoot+'rpjmd/get-data/unit', data)
    .then(response => {
      if(response.data.status){
        this.setState({ dataUnit: response.data.data});
      }
  ...
  handleSubmit = event => {
    ...
    let data = new FormData();
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('Nm_Sub_Unit', this.state.dataPilih.Nm_Sub_Unit);
    data.append('bidang', this.state.dataPilih.Kd_Bidang);
    data.append('urusan', this.state.dataPilih.Kd_Urusan);
    data.append('unit', this.state.dataPilih.Kd_Unit);
    data.append('sub', this.state.dataPilih.Kd_Sub);
      ...
      return (
        <div>
          <FormGroup row>
            ...
            <Col xs="12" md="9">
              <Input type="select" data-number="4" onChange={this.handleChange} value={this.state.dataPilih.Kd_Urusan}>
          ...
          <FormGroup row>
            ...
            <Col xs="12" md="9">
              <Input type="select" data-number="5" onChange={this.handleChange} value={this.state.dataPilih.Kd_Bidang}>
          ...
          <FormGroup row>
            <Col md="3">
              <Label htmlFor="select">Pilih Unit *</Label>
            </Col>
            <Col xs="12" md="9">
              <Input type="select" data-number="6" onChange={this.handleChange} value={this.state.dataPilih.Kd_Unit}>
                <option key="-1" value="">-= Pilih Unit =-</option>
                {this.state.dataUnit.map((data, key) => {
                  return data ? (
                  <option key={key} value={data.Kd_Unit}>{data.Nm_Unit}</option>
                  ) : (null);
                })}
              </Input>
            </Col>
                ...
                <Modal isOpen={this.state.modalCreate} toggle={this.modalCreateClose} className={'modal-lg ' + this.props.className}>
                  ...
                  <Form method="post"  onSubmit={this.handleSubmit} className="form-horizontal" id="form-data">
                    <ModalBody>
                      ...
                      <FormGroup row>
                        <Col md="3">
                          <Label htmlFor="text-input">Nama OPD *</Label>
                        </Col>
                        <Col xs="12" md="9">
                          <Input type="text" id="visi" data-number="1" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.Nm_Sub_Unit} required autoFocus/>
                          <FormText color="muted">Isi Nama OPD</FormText>
                        </Col>
                      </FormGroup>
                    </ModalBody>
                    <ModalFooter>
